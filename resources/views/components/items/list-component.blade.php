            ...
            <div class="accordion" id="accordionPanelsStayOpenExample">
               <form action="{{ route('list', ['cat' => $catSlug, 'type' => $type, 'slug' => $slug]) }}" method="GET">
                  ...
                  <select id="order" name="order" class="card-select">
                     <option value="reference_asc" @if ($order == 'reference_asc') selected @endif>{{ __('item.reference_asc') }}</option>
                     <option value="reference_desc" @if ($order == 'reference_desc') selected @endif>{{ __('item.reference_desc') }}</option>
                     <option value="price_asc" @if ($order == 'price_asc') selected @endif>{{ __('item.price_asc') }}</option>
                     <option value="price_desc" @if ($order == 'price_desc') selected @endif>{{ __('item.price_desc') }}</option>
                  </select>
               ...
               <div style="text-align: right;">
                  @if (count($selected)> 0)
                     <a href="{{ route('list', ['cat' => $catSlug, 'type' => $type, 'slug' => $slug]) }}" class="btn btn-sm btn-info">{{ __('item.reset') }}</a>
                  @endif
                 <br/>
                     ...
                     <div class="accordion-body listing-accordion-body">
                        <ul>
                           <li class="d-flex align-items-center">
                              <input type="checkbox" class="listing-checkbox" name="new" id="new" value="1" @if (isset($new)) checked @endif>
                     ...
                     <div class="accordion-item listing-accordion-item ">
                        <h2 class="accordion-header" id="panelsStayOpen-heading{{ $meta['id'] }}">
                           <button class="accordion-button listing-accordion-button collapsed" type="button"
                                 ...
                                 <span class="listing-accordion-head">{{ ucfirst($meta['name']) }} @if (isset($selected[$meta['id']])) ({{ __('item.filters') }}: {{ count($selected[$meta['id']]) }}) @endif</span>
                           </button>
                        </h2>
                                       ...
                                       <li class="d-flex align-items-center">
                                             @if ($meta['is_color'])
                                                <input type="checkbox" class="listing-checkbox"
                                                   name="metas[{{ $meta['id'] }}][{{ $value['value'] }}]" id="metas[{{ $meta['id'] }}][{{ $value['value'] }}]" value="{{ $value['value'] }}" @if (isset($selected[$meta['id']][$value['value']])) checked @endif><span
                                                   class="select-bg"
                                                   style="background-color: {{ $value['value'] }}; width: 20px; height: 20px; display: inline-block; border: 1px solid #000;"></span>
                                                ({{ $value['count'] }} items)
                                             @else
                                                <input type="checkbox" class="listing-checkbox"
                                                   name="metas[{{ $meta['id'] }}][{{ $value['value'] }}]" id="metas[{{ $meta['id'] }}][{{ $value['value'] }}]" value="{{ $value['value'] }}" @if (isset($selected[$meta['id']][$value['value']])) checked @endif>
